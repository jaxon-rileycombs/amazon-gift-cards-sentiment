...
if (!inputPath || !lexiconPath || !outputPath) throw new Error('Usage: node add-nrc-emotions.mjs INPUT_JSON NRC_LEXICON OUTPUT_JSON');
...
function score(review) {
  ...
  const tokens = `${review.title || ''} ${review.text || ''}`.toLowerCase().match(/[a-z]+(?:'[a-z]+)?/g) ?? [];
  for (const token of tokens) for (const emotion of lexicon.get(token) ?? []) scores[emotion] += 1;
  const top = Math.max(...Object.values(scores));
  return { nrcScores: scores, nrcEmotion: top === 0 ? 'none' : emotions.find((emotion) => scores[emotion] === top) };
}
report.results = report.results.map((review) => ({ ...review, ...score(review) }));
